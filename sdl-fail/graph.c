...
void save_node_array(node_t* nodes, int no_of_nodes, int no_of_edges, const char* pathname)
{
       ...
       size_t filesize = no_of_nodes*sizeof(node_t) + no_of_edges*sizeof(int);
       truncate(pathname, filesize);

       void* mapped_file = mmap(NULL, filesize, PROT_WRITE, MAP_SHARED, fd, 0);
       if (mapped_file == MAP_FAILED){
	       perror("mmap");
	       exit(1);
       }

       int* edges_offset = no_of_nodes*sizeof(node_t);
       int* edges = (int*) mapped_file + edges_offset;
       int edge_counter = 0;
       for (int i = 0; i < no_of_nodes; i++){
	       if (nodes[i].adj_list == NULL) continue;
	       for (link_t* l = nodes[i].adj_list; l != NULL; l = l->next){
		       edges[edge_counter++] = (node_t*)l->data - nodes;
	       }
	       edges[edge_counter].next = NULL;
       }
